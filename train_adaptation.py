...
def get_args():
    ...
    parser.add_argument("--exp", type=str, default="SFDA_ss_IF25")
...
class Trainer:
    ...
    def initialize_prototype(self):
        ...
        memory_image = self.collaboration_module.memory_bank.unsqueeze(0)
        self.writer.add_image("Prototype_Matrix", memory_image, global_step=0)
    ...
    def _do_epoch(self):
        ...
        for iteration, ((img, class_l), d_idx) in enumerate(tqdm(self.target_loader)):
            ...
            self.writer.add_scalar('lora/loss_clip', loss_clip, iter_num)
            self.writer.add_scalar('lora/loss_tc', loss_tc, iter_num)
            self.writer.add_scalar('lora/loss_nce', loss_nce, iter_num)
            self.writer.add_scalar('lora/lr_lora', self.optimizer_lora.param_groups[0]['lr'], iter_num)
        ...
        self.writer.add_scalar('lora/vlm_acc', vlm_accuracy, self.current_epoch)
        ...
        for iteration, ((img, class_l), d_idx) in enumerate(self.target_loader):
            ...
            self.writer.add_scalar('tar/loss_tma', loss_tma, iter_num)
            self.writer.add_scalar('tar/loss_mi', loss_mi, iter_num)
            self.writer.add_scalar('tar/loss_cls', loss_cls, iter_num)
            self.writer.add_scalar('tar/loss_tg', loss_tg, iter_num)
            self.writer.add_scalar('tar/lr_tar', self.optimizer_tar.param_groups[0]['lr'], iter_num)
    ...
    def do_training(self):
        for self.current_epoch in tqdm(range(self.args.epochs)):
            if self.args.dynamic_ad:
                ...
                self.writer.add_scalar('alpha_decay/alpha_decay', self.ad, self.current_epoch)

            tar_accuracy, vlm_accuracy = self._do_epoch()

            prototype_memory = self.collaboration_module.memory_bank.clone().unsqueeze(0)
            self.writer.add_image("Prototype_Matrix", prototype_memory, global_step=self.current_epoch+1)
