...
class Trainer:
    def __init__(self, args, device):
        ...
        self.learning_rate_lora = float(args.learning_rate_lora)
        self.learning_rate_tar = float(args.learning_rate_tar)
        ...
        self.optimizer_tar = optim.AdamW(self.model.parameters(), lr=self.learning_rate_tar,
                                         betas=(0.9, 0.999))
        self.optimizer_lora = optim.AdamW(get_lora_parameters(self.clip_model), lr=self.learning_rate_lora,
                                          betas=(0.9, 0.999))
    ...
    def _do_epoch(self):
        ...
        for iteration, ((img, class_l), d_idx) in enumerate(tqdm(self.target_loader)):
            ...
            self._update_learning_rate(self.optimizer_lora, self.scheduler_lora, self.learning_rate_lora, iter_num)
            wandb.log({
        ...
        for iteration, ((img, class_l), d_idx) in enumerate(self.target_loader):
            ...
            iter_num = iteration + self.current_epoch * len(self.target_loader)
            self._update_learning_rate(self.optimizer_tar, self.scheduler_tar, self.learning_rate_tar, iter_num)
            wandb.log({
    ...
    def do_training(self):
        for self.current_epoch in tqdm(range(self.args.epochs)):
            ...
            if tar_accuracy < self.args.acc_lower_bound:
                logging.info("\n*************Early Stopping!************")
                logging.info(f"Accuracy is lower than {self.args.acc_lower_bound}, early stopping")
                break
...
def train_sweep():
    ...
    with wandb.init() as run:
        ...
        device = torch.device("cuda:" + config.GPU_num if torch.cuda.is_available() else "cpu")
        ...
        get_aug_config(config)
        config.alpha = config.scaling * config.r
        ...
        if os.path.exists(config.output_folder):
            ...
            maybe_mkdir_p(config.output_folder)
            print(">>> Output results will be saved at: {}".format(config.output_folder))
        
        config.checkpoint_path = join(os.getcwd(), 'pretrain', 'ERM', config.backbone, config.dataset, f"{config.source}_best.pth")
        config.lora_cp_path = join(os.getcwd(), 'pretrain', 'LORA', config.CLIP_backbone.replace('/', '-'),
                                    config.dataset, f"{config.source}", 'lora.pt')

        for handler in logging.root.handlers[:]:
            logging.root.removeHandler(handler)
        logging.basicConfig(filename=config.output_folder + "/log.txt", level=logging.INFO,
        ...
        logging.info(f">>> Training on {config.dataset} with pre-trained source domain: {config.source}")
        logging.info(f">>> Adapting to target domain: {config.target}")
