...
def get_args():
    ...
    # Experiment Name
    parser.add_argument("--exp", type=str, default="CLIP-oracle")
...
class Trainer:
    ...
    def _do_epoch(self):
        ...
        # Step1: CLIP LoRA Tuning
        logging.info("\nDoing CLIP LoRA Tuning...")
        for iteration, ((data, class_l), _) in enumerate(self.train_loader):
            data, class_l = data.to(self.device), class_l.to(self.device)

            self.optimizer_lora.zero_grad()
            anchor_prompt = [f"a {self.args.classes[i].replace('_',' ')}" for i in class_l]
            ...
            wandb.log({
                ...
                'stage1_LoRA/step': iteration + self.current_epoch * len(self.train_loader),
        ...
        total_samples = 0
        for iteration, ((data, class_l), _) in enumerate(self.val_loader):
            data, class_l = data.to(self.device), class_l.to(self.device)
            with torch.no_grad():
